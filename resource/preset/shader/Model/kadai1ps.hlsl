...
PixcelShaderOutput main(VertexShaderOutput input)
{
    ...
    float4 baseColor = gMaterial.albedoColor;
    ...
    float3 r = reflect(toCamera, normalize(input.normal));
    
    float4 env = colorMap.Sample(baseColorSmp, r);
    
    output.color = float4(lightFinalColor + env.rgb, baseColor.a);
